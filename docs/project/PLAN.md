...
- preserve the audit hardening now merged on `main`:
  ...
  - PR #103: browser egress is connection-bound to policy-validated literal addresses, evidence capture cannot outlive the run deadline, the documented operator CLI separator form is accepted, the vulnerable Stagehand transitive Undici is patched, and workflow actions are immutable-SHA pinned;
...
- before authorization, run the offline D-043 acceptance preparation preflight against the exact canonical manifest/universe, proposed-vs-current execution profile, source-dated cost plan, positive ceiling/rationale, and human-baseline description; the first packet is frozen at `docs/project/data/gate13-acceptance-packet-2026-09-23.json` with `openai/gpt-5.6-sol`, a $10/brief ceiling, upper-bound source-attributed model/browser accounting, and the normal-tools baseline procedure, and must reproduce `PREPARED_NOT_AUTHORIZED`; preparation evidence is never authorization;
- authorize the complete acceptance candidate manifest through one D-036 atomic approval batch tied to the exact manifest SHA-256; partial target approval is not valid acceptance state, and the authoritative batch/target approval timestamp is assigned only when that atomic batch is durably persisted;
- use the mutation-free acceptance preflight to validate canonical manifest/universe membership, execution profile, cost inputs, and human-baseline description before authorization; use read-only durable readiness to report—but never auto-execute—the next valid transition; use the read-only per-target acceptance worklist to bind operational status to the exact reserved measured run rather than unrelated target history; then use the smallest internal Gate 13 operator surface to derive/freeze acceptance state from canonical manifest + universe + stored batch with a repository/database-owned canonical freeze timestamp, record server-timestamped measured-human baselines, run exactly one frozen target at a time through the existing read-only workflow, require explicit human source/screenshot audit before completed-attempt persistence, snapshot duration + owned action count from the durable run, derive outcome provenance/disposition/duration/action count from durable truth, and evaluate without weakening frozen criteria;
- compare against the existing human researcher using normal tools; scope-matched human restrictions are calibration-only; an independent browser-agent comparator may run only as a separately versioned `AGENT_COMPARISON` experiment with separate authorization/storage, a dedicated unsigned-in mock-keychain Chrome for Testing + BrowserSkill profile, connection-bound egress, a shell-disabled worker with an experiment-owned read-only MCP browser gateway, a frozen descriptive reference-cost plan, and at most one blinded cross-model `MODEL_REVIEWED` evidence-support review per completed attempt; comparator elapsed/cost/review evidence cannot satisfy the Gate 13 human-baseline, source-audit, or human-review acceptance criteria;
...
- freeze a rationale-backed cost ceiling, versioned source-attributed cost plan, **and exact execution profile** before results; the live model name/base URL and Steel base URL plus the checkout's pinned Stagehand dependency, expected immutable Steel image pin, `networkEgressMode=ASTRA_CONNECTION_BOUND_PROXY_V1`, and browser-visible proxy hostname must match `gate13-execution-profile-v2` before research starts; expected Steel pinning must not be misreported as runtime digest attestation; public browser traffic must use the sandbox-owned connection-bound proxy so the actual upstream socket dials a policy-validated literal address; then bind the single durable `AGENT_MODEL_USAGE` step into the research attempt and derive each attempt's model/browser/provider component costs from attempt-bound model usage + server-derived duration + frozen rates (including explicit rounding/source dates), and record the full Astra-side human-time breakdown (target setup, evidence mapping/source audit, corrections/finalization, failure triage, other measured work), correction count/severity, unsupported material claims, requested-field coverage, duration, and complete delivery cost.
...
- audit-hardening regressions for redirect/request policy, connection-bound egress, terminal persistence/supervision/reconciliation, >100-event SSE replay, capture provenance, buyer-visible prohibited prose, and bounded diagnostics/evidence/cleanup remain green;
