...
- compare against the existing human researcher using normal tools; scope-matched human restrictions are calibration-only; an independent browser-agent comparator may run only as a separately versioned `AGENT_COMPARISON` experiment with separate authorization/storage, a dedicated unsigned-in mock-keychain Chrome for Testing + BrowserSkill profile, connection-bound egress, and guarded read-only browser commands, and cannot satisfy the Gate 13 human-baseline or human-review acceptance criteria;
