...
- acceptance preflight must remain mutation-free: offline preparation returns `PREPARED_NOT_AUTHORIZED`, validates exact 43 manifest/universe membership plus live/frozen execution profile and business measurement inputs, and never requires/creates database state; durable readiness is DB-read-only, requires explicit canonical approval-batch identity before freeze, rejects manifest/hash/target drift, and reports the next transition without executing it;
