...
- the checked-in first-acceptance packet must reproduce `PREPARED_NOT_AUTHORIZED` from the canonical 43-target manifest/universe, exact frozen execution profile, source-attributed upper-bound cost plan, $10/brief ceiling/rationale, and normal-tools baseline description; its packet manifest must stay explicitly unauthorized and exact-manifest-SHA bound;
...
- keep any general-purpose browser-agent comparison in a separate protocol/storage boundary: comparator records must never satisfy `MEASURED_HUMAN`, human-review, or original acceptance outcome requirements; deterministic coverage must prove separate authorization, exact first-attempt reservation, duplicate prevention, timeout/interruption preservation, approved-domain evidence, shell-disabled generation through the experiment-owned read-only BrowserSkill MCP gateway plus broker revalidation, dedicated browser readiness, system-Keychain prohibition/mock credential storage, append-only attempt-digest-bound `MODEL_REVIEWED` records, frozen reference-cost-plan hashing/arithmetic, descriptive-only cohort reporting, and absence of dependencies on the original prospect/run PostgreSQL repositories; before live-company authorization, run the real Codex + real BrowserSkill local fixture behind the connection-bound proxy plus the real blinded Luna review fixture, require zero original-Gate mutation, keep `humanBaselineMinutes`/`humanReviewMinutes = null`, and never describe model review as independent source-page verification.
