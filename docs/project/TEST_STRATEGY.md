...
- the acceptance operator path must reconstruct the expected approval batch from the canonical manifest before sample freeze, require exact 43-member universe equality, keep read-only inspection commands migration-free, generate freeze/review timestamps at execution time, keep acceptance baselines `MEASURED_HUMAN`, require explicit human source/screenshot-audit completion before persisting a completed attempt, and derive later outcome baseline/disposition fields from durable sample/baseline/attempt truth rather than operator-supplied provenance;
