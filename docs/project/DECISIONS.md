...
## D-043 — Gate 13 preflight reports readiness but never creates authorization
...
Gate 13 exposes two explicit read-only preflight surfaces before irreversible measured execution:

1. `acceptance-preflight` validates preparation without requiring PostgreSQL. It checks the exact canonical 43-target manifest/universe membership, the operator-supplied frozen execution profile against the live environment/repository pins, the versioned cost plan, a positive cost ceiling, nonblank cost rationale, and nonblank normal-tools human-baseline description. Success returns `PREPARED_NOT_AUTHORIZED`.
2. `acceptance-readiness` reads durable PostgreSQL state without migrations or writes and reports the next valid transition: atomic approval, sample freeze, measured baselines, remaining run/review work, or complete-cohort evaluation.

Before sample freeze, readiness requires an explicit approval-batch ID and verifies that the durable batch is bound to the exact canonical manifest ID/SHA and manifest-derived 43 target snapshots. Once a sample is frozen, readiness derives baseline/outcome counts from that durable sample.

Neither command creates approval, freezes a sample, writes a baseline/outcome, starts a browser, or performs research.

**Why**

By Gate 13 the protocol has several deliberately frozen inputs and ordered durable transitions. Making operators manually infer readiness creates avoidable setup errors; collapsing preparation into authorization would be worse. Separate read-only preflight surfaces reduce execution mistakes while preserving the explicit human approval boundary.

**Consequences**

- a green offline preflight means only that inputs are internally prepared and match the intended live runtime;
- `PREPARED_NOT_AUTHORIZED` must never be interpreted as approval;
- candidate metadata remains non-authoritative;
- durable readiness cannot bless an arbitrary or unrelated approval batch;
- pre-freeze readiness fails rather than guessing when no approval-batch ID is supplied;
- readiness never auto-advances the state machine;
- all irreversible transitions retain their existing explicit confirmation/authorization requirements.

**Revisit when**

A future operator UI may render the same preflight state graph interactively. It must preserve the same separation between read-only readiness and explicit mutating authorization/execution.
