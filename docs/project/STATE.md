...
1. **Product/value risk — current Gate 13 blocker:** the complete 43-member U.S. transportation acceptance universe is frozen and every member now has verified official public domain/start-page **candidate** metadata, but candidate enrichment is not authorization. Gate 13 still needs explicit durable target approval, one durable pre-run measured-human baseline per accepted target, the serial live run, and a complete reviewed outcome set under the frozen v7 total-labor/usability protocol before any value claim.
