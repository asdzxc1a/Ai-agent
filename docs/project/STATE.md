...
1. **Product/value risk — current Gate 13 blocker:** the complete 43-member U.S. transportation acceptance universe, candidate metadata, execution profile, cost plan/ceiling/rationale, and normal-tools human-baseline procedure are now pre-result prepared and the offline preflight is green. That preparation is still not authorization: no real approval batch has been persisted. Gate 13 now needs explicit all-43 operator authorization, one durable pre-run measured-human baseline per target, the serial first-attempt live run, and a complete reviewed outcome set under the frozen v7 total-labor/usability protocol before any value claim.
...
For acceptance, use the complete 43-equity IYT holdings snapshot dated 2026-09-17 as the independent U.S. transportation candidate universe. The checked-in packet at `docs/project/data/gate13-acceptance-packet-2026-09-23.json` has already passed the mutation-free preflight as `PREPARED_NOT_AUTHORIZED` with exact manifest SHA `9b050c12d784f8476ca0f80cbdb9e425b634b9d2c484730ac1617c025dcec109`, model `openai/gpt-5.6-sol`, the v2 connection-bound execution profile, a source-attributed upper-bound delivery-cost plan, a $10/brief ceiling, and the frozen normal-tools baseline procedure. Do not change those packet inputs after result visibility. The next operator action is `pnpm gate13:operator -- preview` to review the exact canonical manifest ID/SHA and all 43 proposed domains/start URLs without mutation. The next operator transition is explicit authorization followed by the guarded `approve` command, which derives one atomic manifest-hash-bound durable approval batch for all 43 members from those exact bytes; if confirmation, validation, or storage fails, none of the batch is authorized. After that batch succeeds, freeze exactly those 43 target IDs with no discretionary exclusions or sampling. The human comparator uses its normal research tools. A member that blocks or lacks usable public access remains in the denominator as explicit failure evidence rather than being removed after results are known. Persist a server-timestamped human baseline record for the frozen sample/target before Astra is allowed to start, then record blind/unblinded review under the frozen `gate13-brief-review-v1` correction-severity rubric. Run serially through: stored approval → frozen-sample membership → durable human baseline → durable one-attempt run reservation → frozen execution-profile-v2 match (including `ASTRA_CONNECTION_BOUND_PROXY_V1` and browser-visible proxy host) → connection-bound read-only model-backed public research → semantically settled capture receipts → evidence/uncertainty review → human source audit → accepted/corrected brief or explicit live failure. Do not retry a frozen acceptance target after its first durable run exists; keep that failed/cancelled/completed first attempt in the denominator, count all Astra-side human labor (target setup, evidence mapping/source audit, corrections/finalization, failure triage, and other measured operator work), and freeze the cost ceiling/rationale before results. China receives a separate acceptance cohort only after its local multi-source source policy is implemented and qualified.
