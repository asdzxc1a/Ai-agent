...
**Overall status:** Gates 0–12 are PASSED. Gate 13 is active in issue #65. PRs #68–#78 provide the approved-research foundation, September audit hardening, a serial read-only operator research composition, and a durable pre-registered measured-sample protocol; PR #103 adds the final pre-acceptance audit hardening for connection-bound browser egress, bounded evidence capture, reproducible operator CLI invocation, dependency/security pinning, and CI supply-chain controls; PR #104 adds an exact reservation-bound per-target worklist; PR #105 makes the atomic real-company approval timestamp server-owned; PR #106 freezes the first pre-result 43-target acceptance packet with GPT-5.6 Sol, upper-bound source-attributed cost accounting, a $10/brief ceiling, and the normal-tools human baseline. The user has now explicitly authorized all 43 manifest members with no exclusions. Atomic approval batch `g13-us-transportation-approval-2026-09-23-v1` is durably persisted against the exact manifest SHA, and acceptance sample `g13-us-transportation-acceptance-2026-09-23-v1` is durably frozen at 43/43 members under `gate13-measured-research-v7`. Gate 13 remains IN_PROGRESS because measured-human baselines are still 0/43, no measured Astra run has been reserved or executed, no material live claim has been human-audited, and no preparation-time/value result exists yet. In parallel, `gate13-agent-comparator-v1` is implemented and fixture-qualified as a separate `MEASURED_AGENT` experiment over the same frozen manifest; it has separate file-backed first-attempt state, a blinded cross-model Luna evidence-support review layer, descriptive-only reporting, and frozen reference-cost accounting, remains `PREPARED_NOT_AUTHORIZED`, and cannot satisfy or advance the human-comparison gate.
...
- PR #103 closes the pre-acceptance audit gaps: screenshot/page-evidence/run-summary work is abort/deadline bounded; the documented pnpm `--` operator form is accepted; public browser egress is routed through an Astra-owned connection-bound proxy that dials a policy-validated literal address; `gate13-execution-profile-v2` freezes the egress mode/browser-visible proxy host; the Stagehand transitive Undici vulnerability is patched by a scoped override; and GitHub Actions are immutable-SHA pinned. Authoritative code/provider evidence is CI `35755221613`, Steel `35755221687`, and Stagehand Steel `35755221646`.
...
A narrow internal Gate 13 operator surface exists at `apps/gate13-operator`. Its offline `acceptance-preflight` validates the canonical 43-target manifest/universe, proposed-vs-current execution profile, source-dated cost plan, positive ceiling/rationale, and human-baseline description without PostgreSQL or browsing and returns `PREPARED_NOT_AUTHORIZED`; its separate DB-read-only `acceptance-readiness` reports the next valid durable transition and independently rechecks canonical approval-batch provenance when supplied. A second read-only `acceptance-worklist` derives each frozen target's exact next legal transition from the durable measured-human baseline, measured-run reservation, the exact reserved run, the matching persisted attempt, and reviewed outcome; it never substitutes a newer unrelated attempt from the same target's calibration/history. Neither preflight authorizes or mutates anything. Its approval preview is mutation-free and computes the exact SHA-256 of the checked-in manifest bytes. Its approval path derives all 43 target records from those same bytes, requires explicit manifest-ID/SHA confirmation plus `--authorize-all-43`, and persists through the atomic PostgreSQL batch boundary. The authoritative batch/target `approvedAt` is assigned by the repository/database at that persistence boundary rather than trusted from the CLI draft, and the operator prints the canonical persisted timestamp. After approval, the same app derives the acceptance sample from the canonical manifest, universe, and stored batch; freezes only the exact 43-member `COMPLETE_UNIVERSE` with repository/database-owned `frozenAt` (draft caller time is ignored); records server-timestamped `MEASURED_HUMAN` baselines; durably reserves one run ID per acceptance sample/target before browser execution and rejects every second measured start for that target; starts one frozen target at a time through the existing read-only workflow; surfaces artifacts for human source/screenshot audit; requires explicit `--human-audit-complete` before completed-attempt persistence; records typed failures; snapshots server-owned run duration plus owned `ACT`/`AGENT_LOOP_ACTION` count into each service-generated attempt; derives outcome provenance/disposition/duration/action count from durable sample + baseline + attempt truth; and reports status/evaluation without silently changing state. Stagehand prompt/completion/reasoning/cached-token counts and inference time are mapped into an owned model-usage snapshot before agent cleanup, persisted as one `AGENT_MODEL_USAGE` run step and in `run-summary.json`, copied into each service-generated durable research attempt, and exposed through the operator `run-usage` command. Duplicate/malformed usage steps fail attempt persistence. This is measured usage evidence, not a dollar-cost estimate. `execution-profile-export --output <path>` writes that current credential-free execution identity as strict parser-ready JSON with exclusive-create semantics, preventing accidental overwrite of a previously frozen profile. Acceptance freezes both a versioned source-attributed delivery-cost plan and an exact execution profile before results. The authoritative sample freeze timestamp is assigned at durable persistence and is the clock used for cost-source and post-freeze attempt chronology. The profile pins the checked-in Stagehand dependency version, model name/base URL, Steel base URL, and the checkout's expected immutable Steel image pin; the image field is explicitly `EXPECTED_IMAGE_PIN_ONLY`, not a claim that endpoint health attested the running container digest. `run-target` resolves the current profile from env + repository pins, refuses to start when it differs, and records the matched profile durably. Outcome component costs/total are derived from the durable attempt's model usage + server-derived duration plus the frozen cost plan, and outcome-context validation recomputes those quantities; reviewer input no longer owns the dollar scalar. Acceptance also freezes the exact four requested business fields and derives covered field IDs/counts from durable report values or explicit unknowns; silent omission is uncovered and failed attempts cover zero. Acceptance review likewise no longer owns end-to-end duration or unauthorized-action count: those values are derived from durable run timestamps/steps and must match the persisted attempt exactly. Self-hosted Steel uses an explicit documented infrastructure allocation rather than managed Steel Cloud pricing. `sample-status` exposes the reserved run ID even before a terminal run has been reviewed into an attempt. Orphan reservation release is explicit, acquires the same PostgreSQL live-ownership lock as `run-target`, and is allowed only when the exact durable run does not exist; a real run, attempt, or outcome makes the reservation non-releasable. Read-only database inspection paths do not auto-run migrations. Live `run-target` additionally holds one database-scoped PostgreSQL advisory ownership lock for its full lifetime, preventing two operator processes from accidentally violating Gate 13's serial/single-owner assumption. This is deliberately narrower than the durable lease/fencing system required for future multi-worker execution. This tooling does **not** itself authorize the real cohort; a batch exists only after the operator deliberately executes that mutation. For the first 43-target cohort, that explicit mutation has now been executed and the exact acceptance sample has been frozen as recorded below.

The first real Gate 13 authorization and sample freeze are now durable, but no real-company browser research has started. Approval batch `g13-us-transportation-approval-2026-09-23-v1` binds all 43 targets to manifest SHA `9b050c12d784f8476ca0f80cbdb9e425b634b9d2c484730ac1617c025dcec109`, with one server-owned `approvedAt = 2026-09-23T22:16:59.658Z` across the batch and target approvals. Acceptance sample `g13-us-transportation-acceptance-2026-09-23-v1` is frozen at 43/43 complete-universe members with server-owned `frozenAt = 2026-09-23T22:17:42.076Z`, exact PR #106 execution/cost/baseline inputs, and zero baselines/reservations/outcomes. Read-only readiness reports `RECORD_MEASURED_HUMAN_BASELINES`, and the reservation-bound worklist reports all 43 targets at `RECORD_MEASURED_HUMAN_BASELINE`. The cleanest experiment ordering is to finish the normal-tools human baseline for all 43 before exposing the human researcher to any measured Astra output. Gate 13 stays serial/single-owner while multi-worker leases/fencing, durable cancellation intent, and centralized Steel endpoint ownership remain later scale work.
...
1. **Product/value risk — current Gate 13 blocker:** the complete 43-member U.S. transportation universe is explicitly authorized and the exact 43/43 acceptance sample is durably frozen under the PR #106 profile/cost/baseline packet. The current blocker is measured human comparison data: baselines are 0/43, so no measured Astra run may start yet. Gate 13 still needs one durable pre-run measured-human baseline per target, the serial first-attempt live runs, and a complete reviewed outcome set under the frozen v7 total-labor/usability protocol before any value claim.
...
6. **Comparator-interpretation risk:** `gate13-agent-comparator-v1` measures a general-purpose browser agent, not a human researcher. Its elapsed time, reference cost, blinded model review, and first-attempt outcomes must remain separate from `MEASURED_HUMAN`, independent source-page/human audit, and the original Gate 13 50% human-time threshold. `MODEL_REVIEWED` means support-by-provided-evidence screening only; it does not prove the evidence source itself.
...
For acceptance, the complete 43-equity IYT holdings snapshot dated 2026-09-17 is now the authorized and frozen U.S. transportation cohort. The checked-in packet at `docs/project/data/gate13-acceptance-packet-2026-09-23.json` remains the exact pre-result contract: manifest SHA `9b050c12d784f8476ca0f80cbdb9e425b634b9d2c484730ac1617c025dcec109`, model `openai/gpt-5.6-sol`, the v2 connection-bound execution profile, upper-bound source-attributed delivery-cost plan, $10/brief ceiling, and normal-tools baseline procedure. Do not change those packet inputs after authorization/result visibility. The immediate work is human measurement: complete and durably record the normal-tools baseline for all 43 frozen targets before showing the human researcher any measured Astra output. After 43/43 baselines exist, run serially through: durable one-attempt run reservation → frozen execution-profile-v2 match (including `ASTRA_CONNECTION_BOUND_PROXY_V1` and browser-visible proxy host) → connection-bound read-only model-backed public research → semantically settled capture receipts → evidence/uncertainty review → human source audit → accepted/corrected brief or explicit live failure → reviewed outcome. Do not retry a frozen acceptance target after its first durable run exists; keep every failed/cancelled/completed first attempt in the denominator and count all Astra-side human labor. China receives a separate acceptance cohort only after its local multi-source source policy is implemented and qualified.

A separate browser-agent comparator may proceed only under `gate13-agent-comparator-v1`: it references the same exact manifest but uses separate authorization/storage, a dedicated unsigned-in Chrome for Testing + BrowserSkill profile, connection-bound egress, a shell-disabled Codex worker with an experiment-owned read-only MCP gateway backed by a guarded BrowserSkill file broker, runner-owned timing, and `humanBaselineMinutes = null`. macOS system Keychain access is explicitly forbidden; comparator Chrome uses mock/basic credential storage and does not borrow the user's normal Chrome credentials. Completed comparator attempts may receive one append-only blinded `gate13-agent-comparator-model-review-v1` review from `gpt-5.6-luna`; that reviewer sees only brief + supplied evidence, is stored as `MODEL_REVIEWED`, keeps `humanReviewMinutes = null`, and does not count as independent source-page or human verification. A frozen reference-cost plan reports `REFERENCE_API_EQUIVALENT_NOT_ACTUAL_BILL` using conservative pre-result rates so efficiency can be compared without pretending Codex subscription usage is an API invoice. Comparator `report` output is descriptive-only and cannot change the Gate 13 evaluator. Real Codex + real BrowserSkill local-fixture acceptance and real Luna review-fixture acceptance are green; the live-company comparator remains `PREPARED_NOT_AUTHORIZED`, and no frozen-company comparator attempt has been reserved or run.
