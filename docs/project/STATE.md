...
6. **Comparator-interpretation risk:** `gate13-agent-comparator-v1` measures a general-purpose browser agent, not a human researcher. Its elapsed time, automated review, and first-attempt outcomes must remain separate from `MEASURED_HUMAN`, human source audit, and the original Gate 13 50% human-time threshold.
...
A separate browser-agent comparator may proceed only under `gate13-agent-comparator-v1`: it references the same exact manifest but uses separate authorization/storage, a dedicated unsigned-in Chrome for Testing + BrowserSkill profile, connection-bound egress, a guarded BrowserSkill file broker, runner-owned timing, and `humanBaselineMinutes = null`. macOS system Keychain access is explicitly forbidden; comparator Chrome uses mock/basic credential storage and does not borrow the user's normal Chrome credentials. It must not record human baselines, reserve Astra acceptance runs, persist original acceptance outcomes, or change the Gate 13 evaluator. Real Codex + real BrowserSkill local-fixture acceptance is green; the live-company comparator remains `PREPARED_NOT_AUTHORIZED`, and no frozen-company comparator attempt has been reserved or run.
