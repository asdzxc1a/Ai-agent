...
1. **Product/value risk — current Gate 13 blocker:** the six-company U.S./China set is calibration/stress evidence only. The first acceptance experiment is the complete 43-member U.S. transportation universe from the frozen 2026-09-17 IYT equity snapshot. Astra still has no human-audited live acceptance cohort, measured total-human-labor reduction, or measured live delivery cost.
...
3. **Comparator-quality risk — controlled by v7:** the human acceptance baseline must preserve the actual structured brief/evidence before Astra starts. Paired human/Astra quality metrics are required so time savings are not interpreted without output-quality context.
4. **Scale ownership risk:** cancellation intent and Steel endpoint exclusivity are not yet durable cross-process leases/fences. Gate 13 therefore stays serial/single-owner; multiple workers require durable ownership/fencing and centralized endpoint allocation.
5. **Future renderer/state-update risk:** PR #74 closes the current `SalesDecision` buyer-visible prose loophole, but any future shipped final-response renderer and durable conversation state update need their own independent output evaluation before release claims.
6. **Agency boundary risk:** authentication, tenant/client ownership, per-client offer/claim configuration, budgets, retention/export/deletion, audit, and recovery must exist before accepting another organization's data.
