...
A separate independent browser-agent comparator is available through `pnpm gate13:comparator -- ...`. It is a different experiment, `gate13-agent-comparator-v1`, and cannot satisfy Gate 13's measured-human or human-review requirements. Its storage, authorization, first-attempt reservations, and terminal attempts are separate from the acceptance PostgreSQL state. It uses Codex CLI `0.154.0` with `gpt-5.6-sol`, BrowserSkill `0.3.1`, a dedicated unsigned-in Chrome for Testing `153.0.8010.52` profile, connection-bound egress, and a guarded BrowserSkill file broker. macOS system Keychain access is explicitly forbidden: the browser is launched with mock/basic credential storage (`--use-mock-keychain`, `--password-store=basic`) and does not borrow normal Chrome credentials. `pnpm gate13:comparator:fixture` runs the real Codex + real BrowserSkill stack only against a local fixture; live frozen-company comparator attempts require their own exact protocol/manifest authorization.
