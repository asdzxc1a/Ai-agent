...
The internal Gate 13 operator surface is intentionally narrow. `pnpm gate13:operator -- preview` reads the exact checked-in 43-target candidate manifest, validates that it is still `NOT_APPROVED` with every row `PENDING_OPERATOR_APPROVAL`, and prints the exact manifest SHA-256 without mutating durable state. The `approve` command derives every `ApprovedResearchTarget` from those exact manifest bytes and refuses persistence unless the operator explicitly confirms the manifest ID, exact SHA-256, and `--authorize-all-43`. After approval, the same operator app derives the acceptance sample from the canonical manifest + frozen universe + stored atomic batch, records server-timestamped measured-human baselines, starts one serial read-only target at a time, surfaces capture artifacts for human audit, persists completed/failed attempts, derives reviewed outcomes from durable baseline/attempt truth, and evaluates only the complete frozen cohort. Live `run-target` also holds one fail-fast PostgreSQL advisory ownership lock for the duration of the process so two operator shells cannot accidentally execute Gate 13 research concurrently; this remains a single-owner guard, not the later multi-worker lease/fencing system. Read-only preview/status/evaluation commands do not auto-run migrations. Do not run the approval mutation or any real-company research command without the corresponding explicit operator authorization and pre-run measurement state.
