...
The internal Gate 13 operator surface is intentionally narrow. Before authorization, `pnpm gate13:operator -- acceptance-preflight` validates the exact canonical 43-target manifest/universe, proposed execution profile versus current env/repository pins, source-dated model/browser cost plan, positive cost ceiling/rationale, and normal-tools baseline description with **no database or browser access**; success is explicitly `PREPARED_NOT_AUTHORIZED`. `acceptance-readiness` is a separate DB-read-only command that validates canonical approval provenance when supplied and reports the next durable transition without executing it. `pnpm gate13:operator -- preview` reads the exact checked-in 43-target candidate manifest, validates that it is still `NOT_APPROVED` with every row `PENDING_OPERATOR_APPROVAL`, and prints the exact manifest SHA-256 without mutating durable state. The `approve` command derives every `ApprovedResearchTarget` from those exact manifest bytes and refuses persistence unless the operator explicitly confirms the manifest ID, exact SHA-256, and `--authorize-all-43`. After approval, the same operator app derives the acceptance sample from the canonical manifest + frozen universe + stored atomic batch, records server-timestamped measured-human baselines, reserves exactly one measured run ID per acceptance sample/target before live browser execution, starts one serial read-only target at a time, surfaces capture artifacts for human audit, persists completed/failed attempts with server-derived run duration and owned-action count, derives reviewed outcomes from durable baseline/attempt truth, and evaluates only the complete frozen cohort. A terminal first run blocks a second acceptance start even before review/persistence; retries are not part of the first v7 acceptance. Live `run-target` also holds one fail-fast PostgreSQL advisory ownership lock for the duration of the process so two operator shells cannot accidentally execute Gate 13 research concurrently; this remains a single-owner guard, not the later multi-worker lease/fencing system. Stagehand token usage is mapped into owned model-usage evidence and captured before cleanup in the durable run step plus `run-summary.json`; `pnpm gate13:operator -- run-usage --run-id <id>` exposes it without converting tokens into an invented dollar cost. `execution-profile-export --output <path>` writes the exact current credential-free profile as parser-ready JSON and refuses to overwrite an existing file, so operators do not need to copy the wrapped preview output by hand. Acceptance sample freeze requires that `--execution-profile-file` plus `--cost-plan-file`: the execution profile pins the checked-in Stagehand dependency version, model name/base URL, Steel base URL, and the checkout's expected immutable Steel image pin; the profile labels that image identity as expected-only rather than claiming the running endpoint's digest was attested. The versioned source-dated rate plan carries explicit model and browser-provider categories. `run-target` compares the live environment with that frozen profile before starting research and persists the matched profile as a durable run step. `record-outcome` reads the durable run summary and derives the persisted component costs + total from that frozen plan; the reviewer JSON no longer supplies `deliveryCostUsd`, `endToEndDurationMs`, `unauthorizedActions`, or requested-field coverage counts; cost/duration/action/coverage evidence comes from the frozen sample plus durable attempt/run truth. `sample-status` surfaces each durable measured-attempt reservation. `release-orphan-attempt-reservation` is explicit recovery only for the narrow case where the reservation exists but the exact durable run ID does not; it refuses release once a run exists. Read-only preview/status/evaluation commands do not auto-run migrations. Do not run the approval mutation or any real-company research command without the corresponding explicit operator authorization and pre-run measurement state.
